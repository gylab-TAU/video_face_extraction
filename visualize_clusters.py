...
def visualize_labels(episode_name, tracks_file, labels_file):
    tracks_time = read_tracks_times(tracks_file)
    labels_tracks = read_labels_tracks(labels_file)
    # Create an new Excel file and add a worksheet.
    xlsx_name = 'data/{}_clustered.xlsx'.format(episode_name)
    ...
    for label in labels_tracks:
        ...
        for tid in labels_tracks[label]:
            ...
            image = f'data/frames/{tid}_{tracks_time[str(tid)]}.jpg'
...
def main():
    ...
    visualize_labels(episode_name, args.tracks_path, args.labels_path)
